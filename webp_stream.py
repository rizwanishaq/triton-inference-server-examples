...
from utils import mat_to_webp_base64, webp_base64_to_mat, unique_sequence_id
...
class WebPStream:
    ...
    def __init__(self, model_name: str = DEFAULT_MODEL_NAME, triton_url: str = DEFAULT_TRITON_URL) -> None:
        ...
        Args:
            model_name (str, optional): Name of the WebP compression model. Defaults to "webp_model".
    ...
    def __call__(self, image: np.ndarray, sequence_start: bool = False, sequence_end: bool = False) -> np.ndarray:
        ...
        """
        image = mat_to_webp_base64(image)
        ...
        output_image = webp_base64_to_mat(output_image)
...
def run(args):
    ...
    with WebPStream(model_name=args.model_name, triton_url=args.triton_url) as stream:
        try:
            while True:
