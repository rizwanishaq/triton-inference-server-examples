...
class TritonClient:
    ...
    def __call__(self, text_input: str) -> (str, float):
        """
...
def process_text(client: TritonClient, text_input: str) -> None:
    """
    Calls the Triton client to process a specific text input.

    Args:
        client (TritonClient): The Triton client object.
        text_input (str): The text input for processing.
    """
    language, score = client(text_input)
    print(
...
if __name__ == "__main__":
    ...
    # Using ThreadPoolExecutor to process text inputs concurrently
    start_time_threading = time.time()
    with ThreadPoolExecutor(max_workers=num_clients) as executor:
        executor.map(process_text, clients, text_inputs)
    end_time_threading = time.time()
    print(
        f"Time taken using threading: {end_time_threading - start_time_threading} seconds")
