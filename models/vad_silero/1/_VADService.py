...
class VoiceActivityDetector:
    ...
    def __init__(self, sampling_rate: int = 16000, model_path: str = "pretrainedmodel/silero_vad.jit"):
        ...
        self.input_audio_buffer: np.ndarray = np.array([], np.int16)
        self.output_audio_buffer: np.ndarray = np.array([], np.int16)
        self.full_audio_buffer: np.ndarray = np.array([], np.int16)
        self.frame_size: int = int(
            0.02 * self.sampling_rate)  # 20ms frame size
        self.start: int = 0
        self.end: int = 0
        self.speech_flag = False
        self.chunk_size = 512  # Number of samples per chunk for processing
    ...
    def buffer_audio(self, chunk: np.ndarray) -> None:
        ...
        self.input_audio_buffer = np.concatenate(
    ...
    def process_speech_dict(self, speech_dict):
        """
        ...
        # when the audi is not speech, so we put this to zero
        # one idea is to add the noise
    ...
    def __call__(self, chunk: np.ndarray) -> np.ndarray:
        ...
        while len(self.input_audio_buffer) >= self.chunk_size:
            chunk_for_processing = self.convert_to_float32(
                self.input_audio_buffer[:self.chunk_size])
            self.input_audio_buffer = self.input_audio_buffer[self.chunk_size:]

            speech_dict = self.vad_iterator(
                chunk_for_processing, return_seconds=False)
